...
from ..models import Property, Room, ChecklistItem, Check, Photo, Issue, CheckType
from ..schemas import (
    PropertyCreate, PropertyResponse, RoomCreate, RoomResponse,
    ChecklistItemCreate, ChecklistItemResponse, CheckCreate, CheckResponse,
    IssueResponse, DamageReportResponse
)
...
async def upload_and_analyze_photo(
    ...
    for missing in analysis.get("missing_items", []):
        ...
        issue = Issue(check_id=check_id, description=f"Missing: {missing}", item_name=missing, estimated_cost=cost, severity="medium")
        db.add(issue)
        issues.append(issue)
...
async def generate_damage_report(property_id: int, checkin_id: int, checkout_id: int, db: AsyncSession = Depends(get_db)):
    # Get checks with photos
    checkin_result = await db.execute(
        select(Check).options(selectinload(Check.photos), selectinload(Check.issues))
...
async def get_cost_history(property_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        ...
        .join(Check).where(Check.property_id == property_id)
